...
function decrypted_text = inverse_hill_cipher(text, key_matrix)
    det_val = round(det(key_matrix));
    inv_det = mod_inv(det_val, 26);
    inv_key_matrix = mod(round(inv(key_matrix) * det_val * inv_det), 26);
    #disp("Inverse Key Matrix:");
    #disp(inv_key_matrix);
    block_size = size(key_matrix, 1);
    text = pad_text(text, block_size);
    decrypted_text = '';
    ...
    for i = 1:block_size:length(text)
        %block = double(text(i:i+block_size-1)) - 97;
        block = double(text(i:min(i+block_size-1,end))) - 97;
...
function inv_matrix = inv_mod(matrix, mod_base)
    det_val = round(det(matrix));
    inv_det = mod_inv(det_val, mod_base);
    adj_matrix = mod(round(det(matrix) * inv(matrix)), mod_base);
...
function decrypted_text = inverse_keyed_transposition_cipher(cipher, key)
    ...
    decrypted_text = reshaped_text(:, inv_order)';
...
% Test Additive Cipher
additive_encrypted = additive_cipher(plaintext, additive_key);
additive_decrypted = inverse_additive_cipher(additive_encrypted, additive_key);
fprintf('Additive Cipher:\nEncrypted: %s\nDecrypted: %s\n\n', additive_encrypted, additive_decrypted);

% Test Hill Cipher
hill_encrypted = hill_cipher(plaintext, hill_key);
hill_decrypted = inverse_hill_cipher(hill_encrypted, hill_key);
fprintf('Hill Cipher:\nEncrypted: %s\nDecrypted: %s\n\n', hill_encrypted, hill_decrypted);

% Test Rail Fence Cipher
rail_fence_encrypted = rail_fence_cipher(plaintext, 2);
rail_fence_decrypted = inverse_rail_fence_cipher(rail_fence_encrypted, 2);
fprintf('Rail Fence Cipher:\nEncrypted: %s\nDecrypted: %s\n\n', rail_fence_encrypted, additive_decrypted);

% Test Keyed Transposition Cipher
keyed_transposition_encrypted = keyed_transposition_cipher(plaintext, transposition_key);
keyed_transposition_decrypted = inverse_keyed_transposition_cipher(keyed_transposition_encrypted, transposition_key);
fprintf('Keyed Transposition Cipher:\nEncrypted: %s\nDecrypted: %s\n\n', keyed_transposition_encrypted, additive_decrypted);
...
% Test the Feistel Cipher
plaintext = 'talha';
key = 114;

% Encrypt
ciphertext = feistel_cipher(plaintext, key, 2);
fprintf('Feistel Cipher:\nCiphertext: %s\n', ciphertext);
